...
function Rating() {
    ...
    const fetchRatingQuestions = async () => {
      try {
        const { data } = await axios.get(
          `/api/question/getRating/${params.tokenID}`
        );
        setSurveyQuestions(data.questions);
      } catch (error) {
        console.log("Error fetching rating questions:", error);
      }
    };
  ...
  const handleSubmit = async () => {
    const payload = {
      ...
      qa: Object.entries(responses).map(([questionId, answer]) => ({
        question: surveyQuestions.find((q) => q.id === parseInt(questionId))
          .question,
        answer: answer,
      })),
    };
        ...
        <div className="survey-questions">
          {surveyQuestions.map((question) => (
            <div className="sq-question" key={question.id}>
              ...
              <Radio.Group onChange={(e) => handleRadioChange(e, question.id)}>
